...
import { MemoryEntry, StorageConfig } from '../types/memory.js';
...
export class FileManager {
  ...
  getConversationMemoryFilePath(conversationId: string): string {
    return path.join(
      ...
      `${this.config.conversationFilePrefix}${conversationId}.json`
    );
  }
   ...
   * 安全地写入 JSON 文件
   */
  async writeJsonFile<T = any>(filePath: string, data: T[]): Promise<void> {
   ...
   * 获取文件统计信息
   */
  async getFileStats(filePath: string): Promise<{ size: number; exists: boolean }> {
    try {
