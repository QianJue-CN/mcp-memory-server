...
export class MemoryManager {
   ...
   * 创建文件夹
   */
  async createFolder(input: CreateFolderInput): Promise<ApiResponse<FolderInfo>> {
    ...
    try {
      ...
      if (existingFolders.success && existingFolders.data) {
        const folderExists = existingFolders.data.folders.some(
          (f) => f.path === validatedInput.folderPath
        );
        if (folderExists) {
          return {
            success: false,
      ...
      logger.error('Failed to create folder', error as Error, { input });
      return {
        success: false,
   ...
   * 删除文件夹
   */
  async deleteFolder(input: DeleteFolderInput): Promise<ApiResponse<{ deletedMemories: number }>> {
    ...
    try {
      ...
      logger.error('Failed to delete folder', error as Error, { input });
      return {
        success: false,
   ...
   * 重命名文件夹
   */
  async renameFolder(input: RenameFolderInput): Promise<ApiResponse<FolderInfo>> {
    ...
    try {
      ...
      // 检查新文件夹名是否已存在
      const newFolderExists = existingFolders.data.folders.some(
        (f) => f.path === validatedInput.newPath
      );
      if (newFolderExists) {
        return {
          success: false,
      ...
      for (const memory of memoriesInFolder) {
        ...
        if (
          metadata.folder === validatedInput.oldPath ||
      ...
      logger.error('Failed to rename folder', error as Error, { input });
      return {
        success: false,
   ...
   * 列出所有文件夹
   */
  async listFolders(): Promise<ApiResponse<FolderListResponse>> {
    try {
      await this.initialize();
      ...
      try {
        const data = await this.fileManager.readJsonFile<FolderInfo>(foldersFilePath);
        folders = data;
      } catch (error) {
        // 文件不存在或读取失败,返回空列表
        folders = [];
   ...
   * 获取文件夹中的所有记忆
   */
  private async getMemoriesInFolder(folderPath: string): Promise<MemoryEntry[]> {
    const allMemoriesResult = await this.readMemories({ limit: 10000 });
    if (!allMemoriesResult.success || !allMemoriesResult.data) {
      return [];
    }
   ...
   * 保存文件夹信息
   */
  private async saveFolderInfo(folderInfo: FolderInfo): Promise<void> {
    const foldersFilePath = path.join(this.config.storagePath, 'folders.json');
    let folders: FolderInfo[] = [];

    try {
      folders = await this.fileManager.readJsonFile<FolderInfo>(foldersFilePath);
    } catch (error) {
      // 文件不存在,创建新的
      folders = [];
    }
   ...
   * 删除文件夹信息
   */
  private async removeFolderInfo(folderPath: string): Promise<void> {
    const foldersFilePath = path.join(this.config.storagePath, 'folders.json');
    let folders: FolderInfo[] = [];
   ...
   * 获取父文件夹路径
   */
  private getParentPath(folderPath: string): string | undefined {
